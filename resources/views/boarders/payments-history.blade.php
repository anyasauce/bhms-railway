                                        ...
                                        <tbody>
                                            @foreach ($payments as $index => $payment)
                                                <tr>
                                                    ...
                                                    <td>{{ \Carbon\Carbon::parse($payment->payment_due_date)->format('F d, Y') }}</td>
                                                    <td>
                                                    ...
                                                    <td>
                                                        <button class="btn btn-primary btn-sm view-receipt" data-bs-toggle="modal"
                                                            data-bs-target="#receiptModal" data-name="{{ $payment->first_name }} {{ $payment->last_name }}"
                                                            data-room="{{ $payment->room_name }}" data-amount="₱{{ number_format($payment->amount, 2) }}"
                                                            data-partial="₱{{ number_format($payment->partial_amount, 2) }}"
                                                            data-description="{{ $payment->description }}"
                                                            data-due="{{ \Carbon\Carbon::parse($payment->payment_due_date)->format('F d, Y') }}"
                                                            data-status="{{ ucfirst($payment->status) }}"
                                                            data-date="{{ $payment->created_at->format('F d, Y h:i A') }}">
                                                            View Receipt
                                                        </button>
                                                    </td>
                                                </tr>
